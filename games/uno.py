...
class UnoManager(GameManager):
    ...
    async def next_turn(self):
        ...
        while next_player.skipped or next_player.should_be_skipped(
                self.game.top_card,
                self.game.preferences_variables,
                self.game.queued_cards
            ):
            next_player.skipped = False
            if (next_player.should_be_skipped(
                self.game.top_card,
                self.game.preferences_variables,
                self.game.queued_cards
            ) and not next_player.skipped):
                #skipped due to contents of queue
                #contents should be applied
                await self.apply_queued_effects(self.game.turn_index)
            self.update_turn_index()
            next_player = self.game.player_data[self.game.turn_order[self.game.turn_index]]
        ...
        await self.current_active_menu.edit(content=self.get_base_menu_string(),
                                            view=self.base_gui)
    async def apply_queued_effects(self,victim_index):
        """
        Iterates through the stacked cards and applies them to the unfortunate fellow.
        """
        victim_user = self.game.turn_order[victim_index]
        victim_name = victim_user.display_name
        victim_unoplayer = self.game.player_data[victim_user]
